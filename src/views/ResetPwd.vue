...
const submitForm = async () => {
    ...
    try {
        await getApi(
            `/reset-password?token=${token}&email=${email}&password=${form.password}`,
        );
...
<template>
    <section class="flex flex-col h-screen">
        <NavBar />
        <form
            class="flex flex-col place-items-center flex-grow"
        ...
        >
            ...
            <StepCard
                class="w-[49rem] h-[20rem] drop-shadow-2xl border-black border-2 flex flex-col"
            >
