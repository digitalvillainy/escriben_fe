		...
		<form class="flex flex-col place-items-center flex-grow" @submit.prevent="submitForm">
			...
			<StepCard class="w-[49rem] h-[30rem] drop-shadow-2xl border-black border-2">
				...
				<div class="flex flex-row justify-between gap-x-2 !mt-1.5">
					...
					<TextInput type="text" label="Username" placeholder="Username" class="w-full relative" v-model="form.username"
				...
				<div class="flex flex-row justify-between gap-x-2 !mt-1.5">
					...
					<TextInput type="password" label="Type Password Again" placeholder="Type Password Again"
						class="w-full relative" v-model="form.confirmPassword" :errors="v$.confirmPassword.$errors">
						<LockIcon class="absolute top-1 right-1.5" />
					</TextInput>
				</div>
